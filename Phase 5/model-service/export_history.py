...
Run on a laptop, next to the Phase 2 model output. It writes two small files
...
WEBSITE_DATA = HERE.parent.parent / "nhl-prediction-website" / "data"
OUT = HERE / "serving"
...
def export_season_history() -> None:
    walk_forward = pd.read_csv(NHL_MODEL / "data" / "walkforward_predictions.csv",
                               parse_dates=["game_date"])
    dataset = pd.read_csv(NHL_MODEL / "data" / "model_dataset.csv",
...
def export_live_replay() -> None:
    ...
    if not source.exists():
        print("live_demo.json not found - run nhl_model/export_site_data.py first")
        return
