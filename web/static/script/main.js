/* ===========================================================
   ...
   All of the data comes from the Flask back end rather than being
   written in here, so this file only has to:
     1. open and close the mobile menu
     2. draw the confidence rings
     3. remember which team the discussion pick buttons chose
     4. ask the server for the next moment of the replayed game
     5. show every kick-off time in the visitor's own time zone
   =========================================================== */
...
function drawFeed(events, homeAbbr) {
  ...
  for (var i = 0; i < newest.length; i++) {
    var event = newest[i];
    var minute = Math.round(event.minute);
    html +=
      '<div class="feed-item">' +
...
function showLiveGame(live) {
  ...
  var period = document.getElementById("livePeriod");
  var clock = document.getElementById("liveClock");
  if (period) { period.textContent = live.period; }
  if (clock) { clock.textContent = live.clock; }
...
/* ask the Flask back end for the next moment of the replayed game */
...
/* ===========================================================
   5) LOCAL KICK-OFF TIMES

   The server sends every time in UTC, because it has no idea where the
   visitor is. The browser does know, so it rewrites them here.
   =========================================================== */
function toLocalTime(text) {
  var moment = new Date(text);
...
for (var u = 0; u < utcElements.length; u++) {
  var raw = utcElements[u].getAttribute("data-utc");
  if (!raw) {
    continue;
  }
  var local = toLocalTime(raw);
  if (local) {
    utcElements[u].textContent = local;
    utcElements[u].title = "Shown in your own time zone";
  }
}
