...
Run after refreshing the data pipeline:
  cd ../pipeline && python main.py --mode upcoming && python main.py --mode build-merged
  cd ../ml && python3 build_model_dataset.py && python3 predict_upcoming.py
"""
...
from train_evaluate import load_dataset, make_models
...
def main() -> None:
    ...
    probs = []
    for _name, model in make_models().items():
        model.fit(X, y)
