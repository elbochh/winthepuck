...
18 GB data pipeline output), and it writes three small files into `serving/`:
  ...
  model_report.json      the honest accuracy numbers from the walk-forward
                         test, so the website can show them
...
DATASET = ML_DIR / "data" / "model_dataset.csv"
WALKFORWARD = ML_DIR / "data" / "walkforward_predictions.csv"
OUT = HERE / "serving"
...
def make_models(seed: int = 7) -> dict:
    """The same three models the walk-forward test measured."""
...
def model_report() -> dict:
    """Real accuracy numbers from the walk-forward test."""
